...
struct ReceiptView: View {
    ...
    private func sendReceipt() {
        ...
        // Build items to share — QR image first, then text
        var items: [Any] = []
        if let qr = qrImage {
            items.append(qr)
        }
        items.append("TrustMesh receipt for: \(actionText)\n\nScan the QR code in the TrustMesh app to verify.")

        // Present the system share sheet via UIKit (avoids sheet-in-sheet freeze)
        let activityVC = UIActivityViewController(activityItems: items, applicationActivities: nil)

        // Pre-fill the recipient if possible
        if isPhoneNumber(recipient) {
            activityVC.setValue(recipient, forKey: "recipient")
        }
